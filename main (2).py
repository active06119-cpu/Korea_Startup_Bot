...
# 전역 객체
db = Database()
# 1. APIClient 초기화 파라미터 수정 (gemini_key 사용)
api = APIClient(
...
async def search_by_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE, keyword: str):
    """키워드로 공고 검색 실행 (기업마당 + K-스타트업 통합 검색)"""
    loading_msg = await (update.message.reply_text(f"'{keyword}' 키워드로 공고를 검색 중입니다...") if update.message else update.callback_query.message.reply_text(f"'{keyword}' 키워드로 공고를 검색 중입니다..."))
    
    all_results = []
    
    # 1. 기업마당 검색 (hashtags 시도)
    biz_results = api.fetch_bizinfo(hashtags=keyword, search_cnt=50)
    # 2. 기업마당 제목 필터링 (hashtags 결과가 부족할 경우 대비)
    if not biz_results:
        biz_results = api.fetch_bizinfo(search_cnt=100)
    ...
    # 3. K-스타트업 검색 및 필터링
    k_results = api.fetch_kstartup(per_page=100)
    for item in k_results:
        title = item.get('biz_pbanc_nm') or item.get('title', '')
        if keyword.lower() in title.lower():
            all_results.append(item)
    ...
    # 최대 10개 표시
    display_results = all_results[:10]
    
    for item in display_results:
        title = item.get('pblancNm') or item.get('biz_pbanc_nm') or item.get('title')
        agency = item.get('jrsdInsttNm') or item.get('author', '기관정보없음')
        pblanc_id = item.get('pblancId') or item.get('seq') or item.get('biz_pbanc_nm') # 고유 ID가 없는 경우 제목 사용
        url = item.get('pblancUrl') or item.get('detl_pg_url') or item.get('link')
        
        # 상세 요약을 위한 데이터 저장
        context.bot_data[f"summary_{pblanc_id}"] = item.get('bsnsSumryCn') or item.get('pbanc_ctnt') or item.get('description', '')
        
        keyboard = []
        # 2. api.openai_client -> api.gemini_client로 변경
        if api.gemini_client:
...
async def summarize_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. api.openai_client -> api.gemini_client로 변경
    if not api.gemini_client:
        await query.answer("현재 요약 기능을 사용할 수 없습니다. (GEMINI_API_KEY 설정 확인 필요)", show_alert=True)
        return
...
async def filter_by_option(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """지역별/분야별 공고 필터링 검색"""
    query = update.callback_query
    option_type = query.data.replace('filter_', '')
    await query.answer()
    
    options = REGIONS if option_type == 'region' else CATEGORIES
    
    keyboard = []
    for i in range(0, len(options), 2):
        row = [InlineKeyboardButton(options[i], callback_data=f"searchopt_{option_type}_{options[i]}")]
        if i+1 < len(options):
            row.append(InlineKeyboardButton(options[i+1], callback_data=f"searchopt_{option_type}_{options[i+1]}"))
        keyboard.append(row)
    
    keyboard.append([InlineKeyboardButton("⬅️ 뒤로가기", callback_data='search_menu')])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"검색할 {option_type}을 선택해주세요.", reply_markup=reply_markup)

async def search_by_option_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """선택한 옵션으로 검색 실행"""
    query = update.callback_query
    _, option_type, value = query.data.split('_', 2)
    await search_by_keyword(update, context, value)
...
def main():
    ...
    # 필터링 검색 핸들러 추가
    application.add_handler(CallbackQueryHandler(filter_by_option, pattern='filter_'))
    application.add_handler(CallbackQueryHandler(search_by_option_execute, pattern='searchopt_'))
